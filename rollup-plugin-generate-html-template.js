    ...
    async generateBundle(outputOptions, bundleInfo) {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          injected = [injected.slice(0, bodyCloseTag), ...bundleKeys.filter(f => path.extname(f) === ".js").map(async b => // For embedContent option, stuff bundle content into HTML directly,
          // otherwise, prepare script with src tag only.
          options.embedContent ? `<script>\n${await lib.readFile(`${outputDir}${path.sep}${prefix || ""}${b}`)}\n</script>` : `<script ${scriptTagAttributes.join(" ")} src="${bundleDirString}${prefix || ""}${b}"></script>\n`), injected.slice(bodyCloseTag, injected.length)].join(""); // write the injected template to a file
