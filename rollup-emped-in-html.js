            ...
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    injected = [
                        ...
                        ...(await Promise.all(bundleKeys
                            .filter(f => path.extname(f) === ".css")
                            .map(
                                async b =>{

                                    if( options.CSSinclude ){

                                        console.log(`${outputDir}${path.sep}${prefix || ""}-----${b}`)

                                        let cssFile_bundle =  await fs.readFile( `${outputDir}${path.sep}${prefix || ""}${b}` )
                                        cssFile_bundle = cssFile_bundle.toString()
                                        let cssFile_custom =  await fs.readFile( `${outputDir}${path.sep}${prefix || ""}${options.CSSinclude}` )
                                        cssFile_custom = cssFile_custom.toString()

                                        return `<style>${cssFile_custom} ${cssFile_bundle}</style>`

                                    } else {
                                        return `<link rel="stylesheet" type="text/css" href="${prefix ||
                                        ""}${b}">\n`
                                    }
                                }
                            ...
                            .map(
                                 async b => {
                                    ...
                                    if (options.embedContent) {
                                        ...
                                        let t = await fs.readFile( `${outputDir}${path.sep}${prefix || ""}${b}` )
                                        t = t.toString()

                                        return `<script>\n${ t }</script>`

                                    } else {
                                        return `<script ${scriptTagAttributes.join(
                                            " "
