<?php
/**
 * @author col.shrapnel@gmail.com && Milosh Slavich
...
/**
 * Filter input by keys in base
 *
 * @param array $base_array - base
 * @param array $intersect_array - array of keys or array with keys that will be subtract from base array
 * @param array $plus_array  - array that will be added to base array/ Base will be saved if eq keys in base and plus
 * @param bool $reverse_intersect  - array that will be added to base array/ Base will be saved if eq keys in base and plus
 * @return array|FALSE whatever mysqli_query returns
 */
function flt_arr_k(array $base_array, array $intersect_array, array $plus_array=[], bool $reverse_intersect=false ): array
{
    if( isset($intersect_array[0]) && count($intersect_array)>0 ) {
        $intersect_array = array_flip($intersect_array);
    }

    if($reverse_intersect) $base_array = array_diff_key($base_array, $intersect_array);
        else  $base_array = array_intersect_key($base_array, $intersect_array);

    return array_merge($plus_array, $base_array);
}

/**
 * Filter input by values in arrays
 *
 * @param array $base_array - base
 * @param array  $intersect_array - array of keys or array with keys that will be subtract from base array
 * @param array $plus_array  - array that will be added to base array/ Base will be saved if eq keys in base and plus
 * @param $reverse_intersect  - array that will be added to base array/ Base will be saved if eq keys in base and plus
 * @return array|FALSE whatever mysqli_query returns
 */
function flt_arr(array $base_array, $intersect_array, array $plus_array=[], $reverse_intersect=false ): array
{
    if($reverse_intersect) $base_array = array_diff($base_array, $intersect_array);
    else  $base_array = array_intersect($base_array, $intersect_array);
...
class SSQL
{
	...
	protected static $emode;
	protected static $exname;
	...
	function __construct($opt = array(), $second='')
	{
        if (!is_array($opt)) {
            if ( !$second ) {
            ...
            $opt = ['db' => $opt, 'user' => $opt, 'pass'=> $second];
        }
		...
		if (isset($opt['mysqli']))
		{
			if ($opt['mysqli'] instanceof mysqli)
			{
				...
				$this->set_error("mysqli option must be valid instance of mysqli class");
			}
		}
		...
		mysqli_set_charset($this->conn, $opt['charset']) or $this->set_error(mysqli_error($this->conn));
	...
	/**
	 * protected static function which actually runs a query against Mysql server.
	 * also logs some stats like profiling info and error message
	 *
	 * @param string $query - a regular SQL query
	 * @return mysqli result resource or FALSE on error
	 */
	protected static function rawQuery($query)
	{
		...
		if (!$res)
		{
			...
			$this->set_error("$error. Full query: [$query]");
		}
		$this->cutStats();
		return $res;
	}

	protected static function prepareQuery($args)
	{
        $this->error = false;
		...
		if ( $pnum != $anum )
		{
			$this->set_error("Number of args ($anum) doesn't match number of placeholders ($pnum) in [$raw]");
		}
	...
	protected static function escapeInt($value)
	{
		...
		if(!is_numeric($value))
		{
			$this->set_error("Integer (?i) placeholder expects numeric value, ".gettype($value)." given");
			return FALSE;
		}
		if (is_float($value))
		{
    ...
    protected static function escapeStringNames($value)
    {
        if ($value === NULL)
        {
            return 'NULL';
	...
	protected static function escapeIdent($value)
	{
		if ($value)
		{
			return "`".str_replace("`","``",$value)."`";
		} else {
			$this->set_error("Empty value for identifier (?n) placeholder");
		}
	}

	protected static function createInsert($data)
	{
		if (!is_array($data))
		{
			$this->set_error("Value for IN (?v) placeholder should be array");
			return;
		}
		if (!$data)
		{
		...
		{
			$query .= $comma.$this->escapeStringNames($value);
			$comma  = ",";
		}
		return $query;
	}

    protected static function createIN($data)
    {
        if (!is_array($data))
        {
            $this->set_error("Value for IN (?a) placeholder should be array");
            return;
        }
        if (!$data)
        {
	...
	protected static function createSET($data)
	{
		if (!is_array($data))
		{
			$this->set_error("SET (?u) placeholder expects array, ".gettype($data)." given");
			return;
		}
		if (!$data)
		{
			$this->set_error("Empty array for SET (?u) placeholder");
			return;
		}
		$query = $comma = '';
